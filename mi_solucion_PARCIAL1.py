...
def columna_n (matriz:list[list[int]],n:int)->list[int]:
    ...
    for i in range(0,len(matriz)):
        #me interesa que vea todas las filas
...
def columnas(matriz:list[list[int]])->list[list[int]]:
    ...
    for n in range(0,len(matriz[0])):
         #estoy viendo cuantas columnas tiene la primera fila, el resto de las filas tiene igual longitud
        colum:list[int]=columna_n(matriz,n)
        res.append(colum)
    return res
...
def columnas_repetidas(matriz:list[list[int]])-> bool:
    cant_columnas = len(columnas(matriz))
    res:bool=True
    for i in range(0,cant_columnas//2,1):
        if columna_n(matriz,i) != columna_n(matriz,((cant_columnas//2)+i)):
            #asi compara la posicion inicial con su equivalente del otro lado de la columna
            res=False
    return res
...
def pertenece (nacion:str,lista:list[int])->bool:
    res:bool=False
...
def posicion (nacion:str,lista:list[int])->int:
    res:int=0
    for n in range(len(lista)):
        if lista[n]==nacion:
            res = n
...
print(posicion("nz",["nz", "sud", "arg", "aus"]))
